...
def loadNocList() -> list:
    ...
    for c1, c8, c10, c15,c22, c23 in zip(sheet_obj.iter_rows(min_col = 1, max_col = 1, min_row = 2, max_row =1031),
        ...
        character["Politics"] = [x.strip() for x in c8[0].value.split(',')]  
        character["Opponent"] = [x.strip() for x in c10[0].value.split(',') if x.strip() != "EMPTY"]  
        character["Domains"] = [x.strip() for x in c15[0].value.split(',')]   
        character["World"] = [x.strip() for x in str(c22[0].value).split(',') if x.strip() != "EMPTY"] 
